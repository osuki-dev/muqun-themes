...
Light compact layouts use their own portrait illustration. Dark compact and regular layouts share the night panorama, with a right-side focal point retaining Mario in compact framing. The mode-specific Home decoration presents the matching scene with contain fit; the shared wallpaper uses 24% strength to protect conversation text. Default Muqun identity remains intact.
...
Home now uses a dedicated transparent pixel castle and moat scene in both modes and responsive sizes. The mushroom garden remains the separate empty-state illustration.
