...
- `assets/home-light.webp`: Original heroic Shiba Inu alpine rescue companion, richly painted russet and cream fur, crimson windblown scarf, olive radio harness and brass beacon. Complete seated dog, alert gaze to the left, warm sunlight, premium hand-inked Japanese anime with painterly cel shading. A dedicated background-extraction edit removed every background plane to real alpha.
...
- `assets/launch-light.webp`: Independent standing pose of the same rescue dog with a coiled climbing rope and carabiner. Complete ears, paws, tail and scarf. Separate from the seated Home composition. Follow-up edit removed the background and replaced the initial cross patch with an original compass triangle.
...
The seven `assets/icon-*.png` images are original geometric drawings by Muqun, rendered from paths at 128 x 128. Rescue-map notation provides a consistent squared stroke language with clear control semantics at 17 pt: return arrow, compass send, rope-loop attachment, create cross with map corners, scanning reticle, hexagonal settings dial and northeast waypoint arrow. They are template masks, so the App supplies the active theme tint in both modes. No SVG is included in the package.
